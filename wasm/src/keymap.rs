...
use crate::types::{KeyInfo, KeySymbol};
    ...
    for x in 0..count {
        ...
        map.insert(
            ...
            KeyInfo {
                ...
                symbol: KeySymbol::new(Some(format!("{}-{}", symbol_prefix, x)), None),
            },
        );
    }
}

fn add_layer_operation(map: &mut HashMap<u16, KeyInfo>, prefix: u16, count: u16, name: &str, symbol_prefix: &str) {
    for x in 0..count {
        let code = prefix + x;
        map.insert(
            code,
            KeyInfo {
                code,
                rmk: format!("{}({})", name, x),
                symbol: KeySymbol::new(None, Some(format!("{}-{}", symbol_prefix, x))),
            },
        );
    }
}

fn add_modifier_key(map: &mut HashMap<u16, KeyInfo>, code: u16, name: &str, symbol: &str) {
    map.insert(
        code,
        KeyInfo {
            code,
            rmk: format!("MT({}, kc)", name),
            symbol: KeySymbol::new(Some(symbol.to_string()), None),
        },
    );
}

fn add_indexed_keys(map: &mut HashMap<u16, KeyInfo>, prefix: u16, count: u16, name: &str, symbol_prefix: &str) {
    for x in 0..count {
        let code = prefix + x;
        map.insert(
            code,
...
static KEYCODEMAP: LazyLock<HashMap<u16, KeyInfo>> = LazyLock::new(|| {
    ...
    for key in KeyCode::iter() {
        map.insert(
            ...
            KeyInfo {
                code: key as u16,
                rmk: format!("{:?}", key),
                symbol: KeySymbol::new(None, Some(format!("{:?}", key))),
            },
        );
    }
    ...
    map.insert(
        ...
        KeyInfo {
            code: 0x0001,
            rmk: "_".to_string(),
            symbol: KeySymbol::new(None, Some("Trns".to_string())),
        },
    );
